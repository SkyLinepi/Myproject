        ...
        private void SetWaves() { 
            // **เพิ่มการตรวจสอบ Null**
             if (spriteShapeController == null) {
                 Debug.LogError("SpriteShapeController is not assigned in the Inspector!");
                 return;
             }
...
                // ตรวจสอบว่า i > 0 ก่อนที่จะเข้าถึง springs[i-1]
                if (i > 0) { 
                    left_deltas[i] = spread * (springs[i].height - springs[i-1].height);
                    springs[i-1].velocity += left_deltas[i];
                }
                // ตรวจสอบว่า i < springs.Count - 1 ก่อนที่จะเข้าถึง springs[i+1]
                if (i < count - 1) { // <-- ให้ใช้ 'count' แทน 'springs.Count - 1' เพื่อความชัดเจน
